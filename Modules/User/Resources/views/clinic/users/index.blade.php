...
@section('title', __('user::clinic.users.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('user::clinic.users.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('clinic.users.create')) }}" class="btn sbold green">
                    ...
                    <div class="portlet-title">
                        <div class="caption font-dark">
                            ...
                            <span class="caption-subject bold uppercase">
                                {{__('user::clinic.users.index.title')}}
                            </span>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('user::clinic.users.datatable.image')}}</th>
                                    <th>{{__('user::clinic.users.datatable.name')}}</th>
                                    <th>{{__('user::clinic.users.datatable.email')}}</th>
                                    <th>{{__('user::clinic.users.datatable.mobile')}}</th>
                                    <th>{{__('user::clinic.users.datatable.created_at')}}</th>
                                    <th>{{__('user::clinic.users.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            @permission('delete_users')
                                <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('clinic.users.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('clinic.users.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
      			...
      			},
            {
              targets: -1,
              width: '13%',
              title: '{{__('user::clinic.users.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("clinic.users.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("clinic.users.destroy", ":id") }}';
