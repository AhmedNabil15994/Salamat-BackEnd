<html>
    @section('title',__('authentication::clinic.login.routes.index'))
    <link rel="stylesheet" href="{{ url('admin/assets/pages/css/login.min.css') }}">
    @include('apps::clinic.layouts._head_ltr')
    <body class="login">
        <div class="content">
            <form class="login-form" action="{{ route('clinic.login') }}" method="POST">
                {{ csrf_field() }}

                <h3 class="form-title font-green">{{ __('authentication::clinic.login.routes.index') }}</h3>
                <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                    <label class="control-label">
                      {{ __('authentication::clinic.login.form.email') }}
                    </label>
                ...
                <div class="form-group{{$errors->has('password') ? ' has-error' : ''}}">
                    <label class="control-label">
                      {{ __('authentication::clinic.login.form.password') }}
                    </label>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn green uppercase">
                      {{ __('authentication::clinic.login.form.btn.login') }}
                    </button>
                </div>
            </form>
        </div>
