...
@section('title', __('offer::clinic.booked_offers.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('offer::clinic.booked_offers.index.title')}}</a>
                </li>
            </ul>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="portlet light boffered">
                                            ...
                                            <div class="form-body">
                                                <div class="row">
                                                    ...
                                                    {{-- <div class="col-md-3">
                                                        <div class="form-group">
                                                            <label class="control-label">
                                                                {{__('offer::clinic.booked_offers.datatable.status')}}
                                                            </label>
                        ...
                        <table class="table table-striped table-boffered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.total')}} - {{ setting('default_currency') }}</th>
                                    <th>{{__('offer::dashboard.booked_offers.datatable.user')}}</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.offer')}}</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.status')}}</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.method')}}</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.created_at')}}</th>
                                    <th>{{__('offer::clinic.booked_offers.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('clinic.booked_offers.deletes')) }}')">
...
<script>
    function tableGenerate(data = '') {
        ...
        var dataTable =
            $('#dataTable').DataTable({
                ajax: {
                    url: "{{ url(route('clinic.booked_offers.datatable')) }}",
                    type: "GET",
                    data: {
                        req: data,
                    },
                ...
                columnDefs: [
                    ...
                    },
                    {
                        targets: -1,
                        width: '13%',
                        title: '{{__('offer::clinic.booked_offers.datatable.options')}}',
                        className: 'dt-center',
                        offerable: false,
                        render: function(data, type, full, meta) {

                            // Show
                            var showUrl = '{{ route("clinic.booked_offers.show", ":id") }}';
