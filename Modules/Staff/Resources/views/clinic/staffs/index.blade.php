...
@section('title', __('staff::clinic.staffs.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('staff::clinic.staffs.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('clinic.staffs.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('staff::clinic.staffs.datatable.image')}}</th>
                                    <th>{{__('staff::clinic.staffs.datatable.name')}}</th>
                                    <th>{{__('staff::clinic.staffs.datatable.email')}}</th>
                                    <th>{{__('staff::clinic.staffs.datatable.mobile')}}</th>
                                    <th>{{__('staff::clinic.staffs.datatable.created_at')}}</th>
                                    <th>{{__('staff::clinic.staffs.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('clinic.staffs.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('clinic.staffs.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
      			...
      			},
            {
              targets: -1,
              width: '13%',
              title: '{{__('staff::clinic.staffs.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("clinic.staffs.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("clinic.staffs.destroy", ":id") }}';
