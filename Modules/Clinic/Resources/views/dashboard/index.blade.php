...
@section('title', __('clinic::dashboard.clinics.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('clinic::dashboard.clinics.routes.index')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.clinics.create')) }}" class="btn sbold green">
                                        <i class="fa fa-plus"></i> {{__('apps::dashboard.buttons.add_new')}}
                                    </a>
                                </div>
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.clinics.sorting')) }}" class="btn sbold blue">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('clinic::dashboard.clinics.datatable.title')}}</th>
                                    <th>{{__('clinic::dashboard.clinics.datatable.status')}}</th>
                                    <th>{{__('clinic::dashboard.clinics.datatable.created_at')}}</th>
                                    <th>{{__('clinic::dashboard.clinics.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            @permission('delete_clinics')
                                <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('dashboard.clinics.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.clinics.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
      			...
      			},
            {
              targets: -1,
              width: '13%',
              title: '{{__('clinic::dashboard.clinics.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.clinics.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.clinics.destroy", ":id") }}';
