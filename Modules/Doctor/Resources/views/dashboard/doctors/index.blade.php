...
@section('title', __('doctor::dashboard.doctors.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('doctor::dashboard.doctors.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.doctors.create')) }}" class="btn sbold green">
                                        <i class="fa fa-plus"></i> {{__('apps::dashboard.buttons.add_new')}}
                                    </a>
                                </div>
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.doctors.sorting')) }}" class="btn sbold blue">
                                        <i class="fa fa-plus"></i> {{__('apps::dashboard.buttons.sorting')}}
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.image')}}</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.name')}}</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.email')}}</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.mobile')}}</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.created_at')}}</th>
                                    <th>{{__('doctor::dashboard.doctors.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            @permission('delete_doctors')
                                <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('dashboard.doctors.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.doctors.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
      			...
      			},
            {
              targets: -1,
              width: '13%',
              title: '{{__('doctor::dashboard.doctors.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.doctors.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.doctors.destroy", ":id") }}';
